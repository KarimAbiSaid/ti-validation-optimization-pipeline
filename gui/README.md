...
- Only commonly-tweaked optimizer/electrode fields are exposed (`postproc`, `cpus`, `focality_threshold`, `hard_roi_constraint`, `no_adjacent_electrodes`, electrode radius/gel/max-current) — everything else is fixed at `generate_configs.py`'s current defaults. The old TesFlex-only fields don't exist in `OptimizerConfig` at all anymore (see the pipeline README).
